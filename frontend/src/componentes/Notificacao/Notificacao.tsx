...
interface NotificacaoComponentProps {
  notificacao: NotificacaoProps;
...
export default function NotificacaoComponent({
  notificacao,
  removeNotificacao,
}: NotificacaoComponentProps) {
  useEffect(() => {
    const timer = setTimeout(() => {
      removeNotificacao(notificacao.id);
    }, 5000);

    return () => clearTimeout(timer);
  }, [notificacao, removeNotificacao]);
  ...
  const { tipo = "info", titulo, mensagem } = notificacao;
  const { icon: Icon, iconClassName } = notificacaoStyles[tipo];
                ...
                <div className="flex items-start">
                  ...
                  <div className="ml-4 flex flex-shrink-0">
                    <button
                      ...
                      onClick={() => removeNotificacao(notificacao.id)}
                    >
